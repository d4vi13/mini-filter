 ...
 *    - NTSTATUS .
 */
static NTSTATUS GetElevationStatus(_Out_ PPROC_INFO ProcInfo, _In_ PFLT_CALLBACK_DATA Data) {
    NTSTATUS Status;
...
NTSTATUS ProcInfoInit(_Out_ PPROC_INFO ProcInfo, _In_ PFLT_CALLBACK_DATA Data) {
    ...
    KdPrint(("ProcINfoInit: %lu %p\n", ProcInfo->PID,ProcInfo->PID));

    Status = GetRequestorSID(&ProcInfo->SID, Data);
    if (!NT_SUCCESS(Status)) {
