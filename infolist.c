...
PINFO_LIST InfoListAlloc(_PoolType_ POOL_TYPE PoolType) {
    ...
    InfoList = ExAllocatePoolWithTag(PoolType, sizeof * InfoList, 'list');
    if(!InfoList) {
        return NULL;
    }
 ...
 *    -
 */
NTSTATUS InfoListAppend(_Inout_ PINFO_LIST InfoList, _In_ PFLT_CALLBACK_DATA Data) {
    
    PINFO Info;

    Info = InfoGet(InfoList->PoolType, FLT_FILE_NAME_QUERY_DEFAULT,Data);
    if(!Info) {
